...
def find_combinations(seq, n, f):
    ...
    [[0, 1], [0, 2], [1, 2]]
    """
    return [list(c) for c in xcombinations(list(range(len(seq))), n) 
...
def get_tile_hog(sit):
    ...
    poss = [[t] for t, ul in list(uses.items()) if len(ul) == 4 and len(set(ul)) > 1]
...
def get_fans(sit):
    ...
    if list(fans.keys()) in [[], ['Flower Tiles']]:
